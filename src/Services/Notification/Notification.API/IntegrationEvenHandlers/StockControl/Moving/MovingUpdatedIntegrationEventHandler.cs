...
public class MovingUpdatedIntegrationEventHandler : IIntegrationEventHandler<MovingUpdatedIntegrationEvent>
{
	...
	private readonly ILogger<MovingUpdatedIntegrationEventHandler> _logger;

	private readonly string _group;

	public MovingUpdatedIntegrationEventHandler(
		...
		ILogger<MovingUpdatedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(MovingUpdatedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(MovingUpdatedIntegrationEvent).FullName!)
			)
		...
		_logger.LogInformation("Отправляем уведомление (id = {id}) обновления перемещения клиентам..", @event.Id);
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("MovingUpdated", new { @event.ProductFlowId, @event.Number });

		_logger.LogInformation("Отправка уведомления (id = {id}) обновления перемещения клиентам выполнена успешно.", @event.Id);
	}
}
