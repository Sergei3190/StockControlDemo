...
namespace Notification.API.IntegrationEvenHandlers.StockControl.Moving;

public class MovingBulkDeletedIntegrationEventHandler : IIntegrationEventHandler<MovingBulkDeletedIntegrationEvent>
{
	...
	private readonly ILogger<MovingBulkDeletedIntegrationEventHandler> _logger;

	private readonly string _group;

	public MovingBulkDeletedIntegrationEventHandler(
		...
		ILogger<MovingBulkDeletedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(MovingBulkDeletedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(MovingBulkDeletedIntegrationEvent).FullName!)
			)
		...
		_logger.LogInformation("Отправляем уведомление (id = {id}) удаления перемещения клиентам..", @event.Id);
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("MovingBulkDeleted", new { @event.Info });

		_logger.LogInformation("Отправка уведомления (id = {id}) массового удаления перемещений клиентам выполнена успешно.", @event.Id);
	}
}
