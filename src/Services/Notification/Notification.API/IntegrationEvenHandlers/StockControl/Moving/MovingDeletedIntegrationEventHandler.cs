...
public class MovingDeletedIntegrationEventHandler : IIntegrationEventHandler<MovingDeletedIntegrationEvent>
{
	...
	private readonly ILogger<MovingDeletedIntegrationEventHandler> _logger;

	private readonly string _group;

	public MovingDeletedIntegrationEventHandler(
		...
		ILogger<MovingDeletedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(MovingDeletedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(MovingDeletedIntegrationEvent).FullName!)
			)
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("MovingDeleted", new { @event.ProductFlowId, @event.Number });

		_logger.LogInformation("Отправка уведомления (id = {id}) удаления перемещения клиентам выполнена успешно.", @event.Id);
	}
}
