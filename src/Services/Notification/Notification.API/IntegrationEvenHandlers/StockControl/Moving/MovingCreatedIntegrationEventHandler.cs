...
public class MovingCreatedIntegrationEventHandler : IIntegrationEventHandler<MovingCreatedIntegrationEvent>
{
	...
	private readonly ILogger<MovingCreatedIntegrationEventHandler> _logger;

	private readonly string _group;

	public MovingCreatedIntegrationEventHandler(
		...
		ILogger<MovingCreatedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(MovingCreatedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(MovingCreatedIntegrationEvent).FullName!)
			)
		...
		_logger.LogInformation("Отправляем уведомление (id = {id}) создания перемещения клиентам..", @event.Id);
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("MovingCreated", new { @event.ProductFlowId, @event.Number });

		_logger.LogInformation("Отправка уведомления (id = {id}) создания перемещения клиентам выполнена успешно.", @event.Id);
	}
}
