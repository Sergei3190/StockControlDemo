...
public class WriteOffDeletedIntegrationEventHandler : IIntegrationEventHandler<WriteOffDeletedIntegrationEvent>
{
	...
	private readonly ILogger<WriteOffDeletedIntegrationEventHandler> _logger;

	private readonly string _group;

	public WriteOffDeletedIntegrationEventHandler(
		...
		ILogger<WriteOffDeletedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(WriteOffDeletedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(WriteOffDeletedIntegrationEvent).FullName!)
			)
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("WriteOffDeleted", new { @event.ProductFlowId, @event.Number });

		_logger.LogInformation("Отправка уведомления (id = {id}) удаления списания клиентам выполнена успешно.", @event.Id);
	}
}
