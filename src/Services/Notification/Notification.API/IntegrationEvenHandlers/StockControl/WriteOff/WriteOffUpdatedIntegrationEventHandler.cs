...
public class WriteOffUpdatedIntegrationEventHandler : IIntegrationEventHandler<WriteOffUpdatedIntegrationEvent>
{
	...
	private readonly ILogger<WriteOffUpdatedIntegrationEventHandler> _logger;

	private readonly string _group;

	public WriteOffUpdatedIntegrationEventHandler(
		...
		ILogger<WriteOffUpdatedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(WriteOffUpdatedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(WriteOffUpdatedIntegrationEvent).FullName!)
			)
		...
		_logger.LogInformation("Отправляем уведомление (id = {id}) обновления списания клиентам..", @event.Id);
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("WriteOffUpdated", new { @event.ProductFlowId, @event.Number });

		_logger.LogInformation("Отправка уведомления (id = {id}) обновления списания клиентам выполнена успешно.", @event.Id);
	}
}
