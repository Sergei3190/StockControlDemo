...
public class WriteOffBulkDeletedIntegrationEventHandler : IIntegrationEventHandler<WriteOffBulkDeletedIntegrationEvent>
{
	...
	private readonly ILogger<WriteOffBulkDeletedIntegrationEventHandler> _logger;

	private readonly string _group;

	public WriteOffBulkDeletedIntegrationEventHandler(
		...
		ILogger<WriteOffBulkDeletedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(WriteOffBulkDeletedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(WriteOffBulkDeletedIntegrationEvent).FullName!)
			)
		...
		_logger.LogInformation("Отправляем уведомление (id = {id}) удаления списания клиентам..", @event.Id);
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("WriteOffBulkDeleted", new { @event.Info });

		_logger.LogInformation("Отправка уведомления (id = {id}) массового удаления списаний клиентам выполнена успешно.", @event.Id);
	}
}
