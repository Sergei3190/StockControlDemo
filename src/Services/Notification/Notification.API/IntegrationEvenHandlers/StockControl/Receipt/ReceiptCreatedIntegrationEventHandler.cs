...
public class ReceiptCreatedIntegrationEventHandler : IIntegrationEventHandler<ReceiptCreatedIntegrationEvent>
{
	...
	private readonly ILogger<ReceiptCreatedIntegrationEventHandler> _logger;

	private readonly string _group;

	public ReceiptCreatedIntegrationEventHandler(
		...
		ILogger<ReceiptCreatedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(ReceiptCreatedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(ReceiptCreatedIntegrationEvent).FullName!)
			)
		...
		_logger.LogInformation("Отправляем уведомление (id = {id}) создания поступления клиентам..", @event.Id);
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("ReceiptCreated", new { @event.ProductFlowId, @event.Number });

		_logger.LogInformation("Отправка уведомления (id = {id}) создания поступления клиентам выполнена успешно.", @event.Id);
	}
}
