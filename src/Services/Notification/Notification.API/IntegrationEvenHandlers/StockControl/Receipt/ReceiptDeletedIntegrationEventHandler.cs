...
public class ReceiptDeletedIntegrationEventHandler : IIntegrationEventHandler<ReceiptDeletedIntegrationEvent>
{
	...
	private readonly ILogger<ReceiptDeletedIntegrationEventHandler> _logger;

	private readonly string _group;

	public ReceiptDeletedIntegrationEventHandler(
		...
		ILogger<ReceiptDeletedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(ReceiptDeletedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(ReceiptDeletedIntegrationEvent).FullName!)
			)
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("ReceiptDeleted", new { @event.ProductFlowId, @event.Number });

		_logger.LogInformation("Отправка уведомления (id = {id}) удаления поступления клиентам выполнена успешно.", @event.Id);
	}
}
