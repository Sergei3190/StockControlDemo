...
public class ReceiptBulkDeletedIntegrationEventHandler : IIntegrationEventHandler<ReceiptBulkDeletedIntegrationEvent>
{
	...
	private readonly ILogger<ReceiptBulkDeletedIntegrationEventHandler> _logger;

	private readonly string _group;

	public ReceiptBulkDeletedIntegrationEventHandler(
		...
		ILogger<ReceiptBulkDeletedIntegrationEventHandler> logger)
	{
	...
	public async Task Handle(ReceiptBulkDeletedIntegrationEvent @event)
	{
		...
		var handledEvent = await _importEventService.CheckIsExistEventLogAsync(
			new EventLogDto(
				...
				EventTypeName: typeof(ReceiptBulkDeletedIntegrationEvent).FullName!)
			)
		...
		_logger.LogInformation("Отправляем уведомление (id = {id}) удаления поступления клиентам..", @event.Id);
		...
		await _hubContext.Clients.GroupExcept(_group, connectionIds).SendAsync("ReceiptBulkDeleted", new { @event.Info });

		_logger.LogInformation("Отправка уведомления (id = {id}) массового удаления поступлений клиентам выполнена успешно.", @event.Id);
	}
}
