...
namespace Identity.API.BackgroundTasks;
...
public class EventPublisherHostedService : BackgroundService
{
	...
	protected override async Task ExecuteAsync(CancellationToken stoppingToken)
	{
		...
		stoppingToken.Register(() => _logger.LogInformation("#EP1 Завершаем работу {EventPublisherHostedService} в {time}",
			typeof(EventPublisherHostedService).FullName, DateTimeOffset.Now.ToLocalTime()));
