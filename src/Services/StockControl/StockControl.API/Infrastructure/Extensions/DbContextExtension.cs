...
{
    public static IServiceCollection AddStockControlDbContext(this IServiceCollection services, IConfiguration configuration)
    {
        ArgumentNullException.ThrowIfNull(services, nameof(services));

        static void ConfigureSqlOptions(SqlServerDbContextOptionsBuilder sqlOptions)
        {
            const string assemblerName = "StockControl.API.DAL";
        ...
        switch (dbType)
        {
            case "DockerDb":
            case "SqlServer":
                services.AddDbContext<StockControlDB>(opt =>
                {
