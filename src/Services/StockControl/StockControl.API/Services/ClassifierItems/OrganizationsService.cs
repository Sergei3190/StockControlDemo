...
public class OrganizationsService : IOrganizationsService
{
	...
	public OrganizationsService(StockControlDB dB, ISaveService<StockControlDB> saveService, ILogger<OrganizationsService> logger, IIdentityService identityService)
	{
		_db = dB;
	...
	public async Task<PaginatedItemsDto<OrganizationDto>> GetListAsync(OrganizationFilterDto filter)
	{
		...
		if (totalItems == 0)
			return new PaginatedItemsDto<OrganizationDto>();
		...
		var dtoItems = entities.Cast<Organization>().Select(u => u.CreateDto()!);

		return new PaginatedItemsDto<OrganizationDto>(filter.Page, filter.PageSize, totalItems, dtoItems);
	}
	...
	public async Task<OrganizationDto?> GetByIdAsync(Guid id)
	{
		...
		if (entity is null)
		{
			_logger.LogWarning("Организация с id: {0} не найдена в БД. Операция чтения невозможна.", id);
			return null;
		}
	...
	public async Task<Guid> CreateAsync(OrganizationDto? dto)
	{
		...
		if (dto.Classifier is null)
		{
			dto.Classifier = Classifier.Classifiers
				.First(t => t.Mnemo == OrganizationMnemo)
				.CreateDto()!;
		}
	...
	public async Task<bool> UpdateAsync(OrganizationDto? dto)
	{
		...
		if (entity is null)
		{
			_logger.LogWarning("Организация с id: {0} не найдена в БД. Операция обновления невозможна.", dto.Id);
			return false;
		}
	...
	public async Task<bool> DeleteAsync(Guid id)
	{
		...
		if (entity is null)
		{
			_logger.LogWarning("Организация с id: {0} не найдена в БД. Операция удаления невозможна.", id);
			return false;
		}
	...
	public async Task<BulkDeleteResultDto> BulkDeleteAsync(params Guid[] ids)
	{
		...
		{
			_logger.LogWarning("Организаций с ids: {0} не найдено в БД. Операция массового удаления невозможна.", string.Join(";", ids));
			return new BulkDeleteResultDto()
			{
				ErrorMessage = new List<string>()
				{
					"Организации не найдена в БД. Операция массового удаления невозможна."
				}
			};
		}
		...
		return new BulkDeleteResultDto()
		{
			...
			{
				Message = $"Организации : {string.Join(",", entities.Select(e => e.Name))} успешно удалена",
	...
	public async Task<IEnumerable<(Guid itemId, string name, string number)>> GetProductFlowNumbersByItemIdAsync(params Guid[] ids)
	{
		...
		if (receiptNumbers.Any())
		{
			_logger.LogInformation("Получены номера поступлений (numbers: {numbers})," +
				...
				ids.Length > 1 ? "задейстованы организации" : "задейстована организация",
				string.Join(",", ids));
	...
	private async Task<IEnumerable<(Guid itemId, string name, string number)>> GetReceiptNumdersAsync(params Guid[] ids)
	{
		return await _db.Receipts
			...
			.Where(r => ids.Contains(r.OrganizationId))
			.AsNoTracking()
			.Select(r => Tuple.Create(r.OrganizationId, r.Organization.Name, r.Number).ToValueTuple())
			.ToArrayAsync()
			.ConfigureAwait(false);
	}
